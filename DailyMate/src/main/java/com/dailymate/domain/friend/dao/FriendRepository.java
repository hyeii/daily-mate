...
public interface FriendRepository extends JpaRepository<Friend, Long> {
    ...
    @Query(value = "SELECT new com.dailymate.domain.friend.dto.FriendInfoDto(u.userId, f.requestDate, u.email, u.nickname, u.image, u.profile)" +
            ...
            "AND f.status = true")
    FriendInfoDto findMyFriendToDto(@Param("userId") Long userId, @Param("friendId") Long friendId);

    // 특정 친구 조회 - 엔티티로 반환
    @Query(value = "SELECT f " +
            "FROM Friend f " +
            "JOIN Users u ON (u.userId = f.fromId AND f.toId = :userId) OR (u.userId = f.toId AND f.fromId = :userId) " +
            "WHERE u.userId = :friendId " +
            "AND f.status = true")
    Friend findMyFriendToEntity(@Param("userId") Long userId, @Param("friendId") Long friendId);
